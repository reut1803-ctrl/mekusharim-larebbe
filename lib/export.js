...
export function buildCleanLines(candidate) {
...
export function buildCleanText(candidate) {
  return buildCleanLines(candidate).join("\n");
}

export async function copyClean(candidate) {
  const text = buildCleanText(candidate);
  await navigator.clipboard.writeText(text);
}

// שיתוף כרטיס מלא (טקסט נקי ומכובד) דרך תפריט השיתוף של המכשיר; גיבוי - העתקה ללוח.
export async function shareClean(candidate) {
  const text = buildCleanText(candidate);
...
export async function downloadPdf(candidate) {
